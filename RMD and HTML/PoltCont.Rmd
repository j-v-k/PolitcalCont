---
title: "2016 Presidential Campaign -  FEC Database Individual Contribution Analysis"
...
EData <- read.csv("C:/Users/James/Documents/GitHub/Election_Data_Edited.csv")
...
This data comes from the Federal Election Commissions Contributor data from all states, found here: http://fec.gov/disclosurep/PDownload.do

The data only includes contributions from individuals counted by the FEC and does not include receipts from other sources. The data also only reflects the early part of the race, with no data included past 9/30/2015.
...
The first barchart shows the amount raised by each candiate. Hillary has raised far more money as counted by this dataset. She is dominating rest of the field with over $60,000,000 raised. The next closest is Jeb Bush with over $20 Milllion raised. 
...
This bar chart shows the amount raised per state. Unsurprisingly California has the largest amount of total donations as California has the largest total population. California is followed by Texas then New york. 
...
Below is a chart of the top 8 candidates over time. Interestingly there is not much of an obvious distinction between the candidates trends. All the candidates have similiar paths and increases in funding, with no obvious jumps.   
...
This chart shows the portion of retired persons who make up their total contriubtion base. Ben Carson has the highest portion with almost 30% of his contributions coming from retired persons. Donald Trump and Carly Fiorina are next at above 25% of the total contriubtions coming from retired persons
...
First I want to study Washington D.C's funding correlation with overall funding.
Like the retired plot I made a bar graph showing the percent of contribution total which comes from Washington D.C
...
There looks to be a stronger correlation with percent of contriubtions from Washignton DC then with retired persons, with every extra .01 % from Washington DC leading to an expected increase of around $7M in total contributions. The R Squared here is not too high, but is not weak enough to be dicounted completely:
...
Here we can see that the DC area funding correlates more with overall candidate funding then any other State/Region which would make sense as it is the center of politics in the United States I limited the values shown to RSquared values over .05.
...
Here is a graph of the RSquared data for each city. Washington DC still shows the highest R Squared value. Surprisingly to me, Mineapolis is the second highest. 
...
I want to further narrow down which areas correlate the most with overall funding, so I will now repeat the process but with Zip Codes. Here is a scatter plot. I made the cut to be counted as .5% rather then 1.5% because Zip Codes are smaller and naturally will make a smaller part of the Total Contributions. For some context 10024 is the Zip Code for the upper West Side in Manhattan. 77024 is a Zip Code in Houston, two miles away from where Ted Cruz went to High School.  
...
Without Clinton, the correlation is much less strong. It would be interesting to compare this to the data in the 2004 and 2008 election and before to see if these Zip Codes correlate with mainstream well funded Democrats(essentially only Hilary in this election) or if they are simply very pro Hilary and would not have a strong correlation in elections not involving Hilary. 
...
####Description One:
Plot one shows the percent of a candidates total contribution which comes from each particular state. You can see that the generally the less popular candiadtes generally have contributions coming from mostly there home state(eg. Bobby Jindal has 80% of his contributions coming from Louisiana). Candidates with larger total contriubitons have a more diversified base of support. For instance Hilary has significant portions of revenue (defined as over 2.5% of her total) coming from 15 states. 
...
####Description Two:
This graph shows the "Coefficient of determination" or R Squared Score betweeen  contributions for a candidate which come from that particular city, and the Total_contriubitons overall. In other words this score shows the proportion of the differences in the Total Contributions for each candidate, which can be predicted by the percentage of Total Contriubtions each candidate gets from the respective city.

Washington DC unsurprisingly has the highest Rsquared value, which makes sense given the fact that DC is the seat of American politics. Minneapolis is second for less clear reasons. Surprisingly New York barely makes the .05 threshold for appearing on the graph. The inclusion of Salt Lake City at a .16 value is logical as it is the center for the Mormon Faith, and would most likely be a good indicator of the contribtions of the entire Mormon community. 
...
It was also interesting that the candidates with the highest level of retired support were the three outisiders on the republican side, Carly Fiorina, Donald Trump and Ben Carson. This could possibly indicate that retired persons are likely very disastisfied with the Republican establishment, as these three candidates are not establishment republicans and are classified as outsiders. 

I thought the location analysis was especially interesting. The fact that Washington DC's funding correlated highly with total funding made a lot of sense. It was interesting to see that as a whole NYC's, but that one of it's Zip Codes was. 

In the future I would like to compare the correlation of the funding totals of different locations on Republicans and Democrats sepearte total funding numbers. I would also like to look at which jobs are the most influential, and map rSquared values as I did with location. I would also like to map the infulence of these locations and jobs on actuall poll numbers and votes, not just overall contriubtions.

It is important to note that these amounts only cover the money going to the candidates themselves. The candidates also have Superpacs which they are not officially allowed to control. These Superpacs are allowed to take unlimited amount of campaign contributions and as a result often have even more money then the individual campaigns. 
